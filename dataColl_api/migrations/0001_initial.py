# Generated by Django 4.2.11 on 2024-09-04 13:10

from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='DC_Cases',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('CASE_NUM', models.CharField(blank=True, max_length=6, null=True)),
                ('APP_ID', models.CharField(max_length=6)),
                ('PLAN_ID', models.CharField(max_length=6)),
            ],
        ),
        migrations.CreateModel(
            name='DC_Income',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('EMP_INCOME', models.CharField(max_length=6)),
                ('PROPERTY_INCOME', models.CharField(max_length=6)),
                ('CASE_NUM', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to='dataColl_api.dc_cases')),
            ],
        ),
    ]
