...
const Sidebar = ({ onSelectMenu }) => {
    ...
    useEffect(() => {
        ...
        const filteredMenus = userRole === 'admin' ? allMenus : allMenus.filter(item => item.text !== 'Admin');

        setMenus(filteredMenus);
    }, []);

    // Handle click to show/hide submenu
    const handleClick = (index, e, item) => {
        e.preventDefault();

        // If the clicked item has no submenu, trigger the onSelectMenu
        if (item.submenu.length === 0) {
            onSelectMenu(item.text);
            ...
            {menus.map((item, index) => (
                <li key={index} className='list-item'>
                    <a
                        ...
                        href="#"
                        onClick={(e) => handleClick(index, e, item)}
                    >
                    ...
                    {activeIndex === index && item.submenu.length > 0 && (
                        <ul className={`submenu ${submenuVisible ? 'visible' : ''} list-unstyled`}>
                            {item.submenu.map((submenuItem, subIndex) => (
                                <div onClick={() => onSelectMenu(submenuItem.trim().replaceAll(' ', ''))} style={{ fontSize: "1rem", cursor: "pointer" }} className='text-decoration-none text-black' key={subIndex}>
                                    <li className='p-2 new-li'>{submenuItem}</li>
                                </div>
                            ))}
                        </ul>
                    )}
